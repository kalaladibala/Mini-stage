...
//     $rqt = 'UPDATE t_etablissement e
//             INNER JOIN t_compte AS c ON e.idetab = c.idetab
//             SET e.important=?, e.important2=?
//             WHERE c.idcompte = ?';

//     $stmt = mysqli_prepare($mysqli, $rqt);
//     mysqli_stmt_bind_param($stmt, "ssi", $_POST['important'], $_POST['important2'], $_SESSION['IdUtilisateur']);
...
function modif_important(){
    ...
    $important = mysqli_real_escape_string($mysqli, $_POST['important']);
    $important2 = mysqli_real_escape_string($mysqli, $_POST['important2']);
    $idUtilisateur = mysqli_real_escape_string($mysqli, $_SESSION['IdUtilisateur']);

    $rqt = 'UPDATE t_etablissement e
            INNER JOIN t_compte c ON e.idetab = c.idetab
            SET e.important="' . $important . '", e.important2="' . $important2 . '"
            WHERE c.idcompte =' . $idUtilisateur;
