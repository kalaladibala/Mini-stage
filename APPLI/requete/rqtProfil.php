...
function inscription(){
    ...
    $academiemin = strtolower($academie['nom_academie']);	//academie en miniscule pour l'adresse mail
    $RNEmin= strtolower($_POST['rne']);
    $mailetab = 'ce.'.$RNEmin.'@ac-'.$academiemin.'.fr';

    //insertion
    $rqt='INSERT INTO `t_etablissement`(`nometab`,`idtypeetab`,`ville`,`adresse`,`mailetab`,`idacademie`,`cp`,`logo`,`cachet`,`important`, `RNE`) 
	 VALUES("'.$_POST['etab'].'",'.$_POST['type'].',"'.$_POST['ville'].'","'.$_POST['adresse'].'","'.$mailetab.'", '.$_POST['academie'].', '.$_POST['cp'].',"","","", "'.$_POST['rne'].'")';

    mysqli_query($mysqli,$rqt) or exit(mysqli_error($mysqli));

$nomEtablissement = $_POST['etab'];
$rqt = "SELECT idetab FROM t_etablissement WHERE nometab = '$nomEtablissement'";
$resultat = $mysqli->query($rqt);

if ($resultat) {
    $row = $resultat->fetch_assoc(); // Récupérer la ligne de résultat
    $idEtablissement = $row['idetab']; // Récupérer la valeur de idetab

    $rqt2 = 'INSERT INTO `t_compte`(`idcompte`,`identifiant`,`mdp`,`idprofil`,`nom_compte`,`prenom_compte`,`mail_compte`,`idfonction`,`tel`,`idetab`) 
             VALUES (NULL, "'.$_POST['rne'].'", "'.$pass.'", 4, "'.$_POST['nom'].'", "'.$_POST['prenom'].'", "'.$_POST['mail'].'",
             '.$_POST['fonction'].', "'.$_POST['tel'].'", "'.$idEtablissement.'")';

    mysqli_query($mysqli, $rqt2) or exit(mysqli_error($mysqli));
} else {
    // Gérer l'erreur si la requête de sélection échoue
    echo "Erreur lors de la sélection de l'établissement.";
}
